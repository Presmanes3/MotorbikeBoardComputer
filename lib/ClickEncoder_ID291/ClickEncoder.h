...
class ClickEncoder {
  ...
  public:
  void setButtonHeldEnabled(const bool& d) { buttonHeldEnabled = d; }

  const bool getButtonHeldEnabled( ) { return buttonHeldEnabled; }

  public:
  void setButtonOnPinZeroEnabled(const bool& d) { buttonOnPinZeroEnabled = d; }

  const bool getButtonOnPinZeroEnabled( ) { return buttonOnPinZeroEnabled; }
#endif

  public:
  void setAccelerationEnabled(const bool& a) {
    accelerationEnabled = a;
    if(accelerationEnabled == false) { acceleration = 0; }
  }
...
class AnalogButton : public ClickEncoder {
  public:
  explicit AnalogButton(int8_t BTN, int16_t rangeLow,
                        int16_t rangeHigh); // Constructor for using analog
                                            // input range as a button
};

class DigitalButton : public ClickEncoder {
  public:
  explicit DigitalButton(
      int8_t BTN, bool active = false); // Constructor for using a button only
};
